...
var gemtextPage = template.Must(template.
	...
	Funcs(template.FuncMap{
		...
		"url": func(ctx *GemtextContext, s string) template.URL {
			...
			if ctx.DisableExternalProxy || (u.Scheme != "" && u.Scheme != "gemini") {
...
<details>
	...
	<p>Be advised that no attempt was made to verify the remote SSL certificate.</p>
</details>
`))
...
func main() {
	...
	opts, optind, err := getopt.Getopts(os.Args, "Pb:s:e:")
	...
	http.Handle("/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		proxyGemini(req, false, root, w, r, css, external, disableExternalProxy)
	}))
	...
	http.Handle("/x/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if path[2] != root.Host && disableExternalProxy {
			w.WriteHeader(http.StatusMethodNotAllowed)
			w.Write([]byte("404 Not found"))
			return
		}
		...
		proxyGemini(req, true, root, w, r, css, external, disableExternalProxy)
	}))
